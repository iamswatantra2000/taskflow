    ...
    loading: () => (
      <div className="grid grid-cols-4 gap-3">
        {["Todo", "In progress", "In review", "Done"].map((col) => (
          <div
            key={col}
            className="bg-[var(--tf-bg-card)] border border-[var(--tf-border-subtle)] rounded-[10px] p-3 min-h-[200px] animate-pulse"
          >
            <div className="flex items-center gap-1.5 mb-3">
              <div className="w-[7px] h-[7px] rounded-full bg-slate-200 dark:bg-[var(--tf-bg-dropdown)]" />
              <div className="h-3 w-16 bg-slate-100 dark:bg-[var(--tf-bg-dropdown)] rounded" />
            </div>
          </div>
        ))}
      </div>
