      ...
      <div className="max-w-5xl mx-auto">
        ...
        <AnimateOnView className="text-center mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 border border-slate-200 dark:border-white/8 bg-slate-50 dark:bg-white/[0.02] rounded-full px-4 py-1.5 mb-5">
            <span className="text-[11.5px] font-medium text-slate-500 dark:text-[#555]">Pricing</span>
          </div>
              ...
              <div
                className={`relative rounded-[16px] p-5 sm:p-6 border flex flex-col gap-5 ${
                  plan.popular
                    ...
                    : "border-slate-100 dark:border-white/[0.06] bg-white dark:bg-[#0c0c0c]"
                }`}
              >
                {plan.popular && (
                ...
                <div>
                  <p className="text-[12px] font-medium text-slate-500 dark:text-[#555] mb-2.5">{plan.name}</p>
                  <div className="flex items-baseline gap-1.5 mb-2">
                    <span className="text-[38px] font-bold text-slate-900 dark:text-white">{plan.price}</span>
                    <span className="text-[11px] text-slate-400 dark:text-[#444]">{plan.period}</span>
                  </div>
                  <p className="text-[12.5px] text-slate-500 dark:text-[#555] leading-relaxed">{plan.description}</p>
                </div>
                ...
                <Link
                  ...
                  className={`w-full h-8 flex items-center justify-center text-[12.5px] font-semibold rounded-[8px] border transition-all duration-150 active:translate-y-[3px] active:shadow-none whitespace-nowrap ${
                    plan.popular
                      ...
                      : "bg-white dark:bg-[#111] hover:bg-slate-50 dark:hover:bg-[#161616] text-slate-500 dark:text-[#888] hover:text-slate-900 dark:hover:text-white border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/18 shadow-[0_3px_0_0_rgba(0,0,0,0.12)] dark:shadow-[0_3px_0_0_rgba(0,0,0,0.55)]"
                  }`}
                >
                  {plan.cta}
                </Link>
                    ...
                    <li key={f} className="flex items-center gap-2.5">
                      <div
                        className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${
                          plan.popular ? "bg-indigo-500/15" : "bg-slate-100 dark:bg-white/[0.04]"
                        }`}
                      >
                        <Check size={10} className={plan.popular ? "text-indigo-400" : "text-slate-400 dark:text-[#444]"} />
                      </div>
                      <span className="text-[12.5px] text-slate-500 dark:text-[#666]">{f}</span>
                    </li>
                  ))}
                </ul>
              </div>
