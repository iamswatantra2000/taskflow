...
const ROLE_META = {
  MEMBER: {
    ...
    icon:  User,
    chip:  "bg-white/[0.04] text-[#666] border-white/[0.08]",
  },
  ADMIN: {
    label: "Admin",
    desc:  "Can manage projects and invite members",
    icon:  Shield,
    chip:  "bg-indigo-500/[0.1] text-indigo-300 border-indigo-500/25",
  },
}
...
export function InviteModal({ onClose, pendingInvites, userRole }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div
        className="relative w-full max-w-[480px] bg-[#0d0d0d] border border-white/[0.08] rounded-[20px] overflow-hidden"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[14px] font-bold text-white tracking-tight">Invite member</h2>
              <p className="text-[11.5px] text-[#3a3a3a]">Send an invite link via email</p>
            </div>
          </div>
          <button
            type="button"
            onClick={close}
            className="w-7 h-7 flex items-center justify-center rounded-[8px] text-[#333] hover:text-[#888] hover:bg-white/[0.05] transition-all"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5">
          {!sent ? (
            ...
            <>
              ...
              <div className="space-y-1.5">
                <p className="text-[11px] font-semibold text-[#3a3a3a] uppercase tracking-[0.08em]">Email address</p>
                <div className="relative">
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full bg-white/[0.03] border border-white/[0.08] rounded-[10px] pl-9 pr-4 py-2.5 text-[13px] text-[#ccc] placeholder-[#2a2a2a] outline-none
                      hover:border-white/[0.12] focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/[0.1] transition-all duration-150"
                  />
                </div>
              </div>
              ...
              {/* Role selector */}
              <div className="space-y-1.5">
                <p className="text-[11px] font-semibold text-[#3a3a3a] uppercase tracking-[0.08em]">Role</p>
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-start gap-2.5 p-3 rounded-[10px] border text-left transition-all duration-150
                          ${active
                            ? "bg-indigo-500/[0.08] border-indigo-500/25 ring-1 ring-indigo-500/15"
                      ...
                      >
                        <div className={`w-6 h-6 rounded-[6px] flex items-center justify-center flex-shrink-0 mt-0.5
                          ${active ? "bg-indigo-500/[0.15] border border-indigo-500/25" : "bg-white/[0.04] border border-white/[0.07]"}`}
                        >
                          <RIcon size={12} className={active ? "text-indigo-400" : "text-[#444]"} />
                        </div>
                        <div>
                          <p className={`text-[12.5px] font-semibold ${active ? "text-indigo-300" : "text-[#666]"}`}>
                            {meta.label}
                          </p>
                          <p className="text-[10.5px] text-[#333] mt-0.5">{meta.desc}</p>
                        </div>
                      </button>
                    )
                  })}
            ...
            <div className="text-center py-2">
              ...
              <h3 className="text-[16px] font-bold text-white tracking-tight mb-1">Invite sent!</h3>
              <p className="text-[13px] text-[#555] mb-5">
                <span className="text-[#777]">{email}</span> will receive an email with the invite link.
              </p>

              {/* Copyable link */}
              <div className="bg-white/[0.02] border border-white/[0.06] rounded-[10px] p-3 mb-4">
                <p className="text-[10.5px] font-semibold text-[#2e2e2e] uppercase tracking-[0.08em] mb-1.5">Invite link</p>
                <div className="flex items-center gap-2">
                  <p className="flex-1 text-[11.5px] text-[#555] truncate font-mono">{inviteUrl}</p>
                  <button
                    type="button"
                    onClick={copyLink}
                    className="flex items-center gap-1.5 h-7 px-3 text-[11px] font-semibold rounded-[7px] flex-shrink-0
                      bg-white/[0.05] hover:bg-white/[0.08] border border-white/[0.08] text-[#666] hover:text-white transition-all"
                  >
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={resetForm}
                  className="flex-1 h-9 text-[12.5px] font-semibold text-[#555] hover:text-white
                    bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.07]
          ...
          {pendingInvites.length > 0 && (
            <div className="pt-2 border-t border-white/[0.05]">
              <p className="text-[11px] font-semibold text-[#2e2e2e] uppercase tracking-[0.08em] mb-3">
              ...
              <div className="space-y-1.5 max-h-[160px] overflow-y-auto">
                {pendingInvites.map((inv) => {
                  ...
                  return (
                    <div
                      key={inv.id}
                      className="flex items-center gap-3 p-2.5 rounded-[9px] bg-white/[0.02] border border-white/[0.05]"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12.5px] font-semibold text-[#888] truncate">{inv.email}</p>
                      ...
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center rounded-[6px] text-[#333] hover:text-red-400 hover:bg-red-500/[0.08] transition-all flex-shrink-0"
                        title="Revoke invite"
                      >
                        {revoking === inv.id
